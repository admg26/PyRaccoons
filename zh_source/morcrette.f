       PROGRAM NEWRAD
C
C  Driving program for the extended atmospheric radiation scheme of Morcrette.
       INCLUDE 'zradpar60.f'
C
C          1.     DEFINITION
C                 -----------
...
C definitions for read in
      ...
      DOUBLE PRECISION DUMMY, T(NLEV), H2O(NLEV), O3(NLEV)
     *                ,ZTM1(NLON,NLEV)
...
C
      ...
      INTEGER MMNTH(12), JDAY
      DATA MMNTH/15,45,74,105,135,166,196,227,258,288,319,349/
      DOUBLE PRECISION YTIME
      character*200 base_root
      character*200 input_file,filename_sw, filename_lw
      character*2 filedm(3)
      dimension idaysinm(12), isumdays(12)
      integer iday, imonth 
      data idaysinm /31,28,31,30,31,30,31,31,30,31,30,31/
      isumdays(:) = 0
      isumdays(1) = idaysinm(1)
      do 1300 id=2,12
        isumdays(id)= idaysinm(id) + isumdays(id-1)    
 1300 continue


      ITASK=3

      OPEN(27,FILE='FLUXES_LW',STATUS='UNKNOWN', FORM='FORMATTED')
      OPEN(28,FILE='FLUXES_SW',STATUS='UNKNOWN', FORM='FORMATTED')
...
C      CLOSE(13)
      ...
      if (imonth.eq.1) then
		    ...
		    jday = iday + isumdays(imonth-1)
      end if
  
C      OPEN(15,FILE='in_lat',STATUS='OLD',FORM='FORMATTED')
C      READ(15,*) XLATITUDE
C      CLOSE(15)
C
C
C      OPEN(17,FILE='albedo.dat',STATUS='OLD',FORM='FORMATTED')
C      READ(17,*) ALBMPAR
C      CLOSE(17)
C 
C      OPEN(18,FILE='szen_in',STATUS='OLD',FORM='FORMATTED')
C      READ(18,*) AMU0
C      CLOSE(18)
...
C Set up latitude grid
      ...
      IF((NLAT/2)*2 .EQ. NLAT) THEN
        ZDLAT = 3.14159/(NLAT)
        DO 30 ILAT=1,NLAT
          XLAT(ILAT) = -1.57079 + ZDLAT*((ILAT)-0.5)
 30    CONTINUE
      ELSE IF (NLAT.NE.1) THEN
        ZDLAT = 3.14159/(NLAT-1.)
        DO 301 ILAT=1,NLAT
         XLAT(ILAT) = -1.57079 + ((ILAT)-1.)*ZDLAT
 301  CONTINUE
      ELSE
        XLAT(1)=0.523600
      END IF
C
      ZDEGRAD = 0.017453292
      AMU0=cos(AMU0*ZDEGRAD)

      XLAT(1)=XLATITUDE*ZDEGRAD
C calculate day
C
C      JDAY = MMNTH(ITIM)
c      CALL CALNDR(JDAY)
      YTIME = (DFLOAT(JDAY-1)+YCLOCK/2./API)/YEARL*2.*API
C
...
C     
      ...
      DO 36 ILST=1,NLON
        DO 35 ILHT=1,NRLEV
          JKL = NRLEV + 1 - ILHT
          READ(11,*)I,T(JKL),H2O(JKL),APM1(ILST,JKL),O3(JKL)
          IF (JKL .NE. NRLEV) THEN
            APHM1(ILST,JKL+1) = (APM1(ILST,JKL+1)+APM1(ILST,JKL))/2.
          END IF
 35    CONTINUE
C
       APHM1(ILST,NRLEVP1)=101301.
...
C
 ...
 39   CONTINUE
      ...
      DATA ZSNOWAL/0.01/
C
C  remark:  ZCARDI is the CO2 concentration in MMR!
C
      DATA ZCARDI/5.469092164307E-4/
      DATA ZEMISS/0.996/
      DATA ZSUPSAT/0.01/
...
C
  220 CONTINUE
C
C      CALL SOLANG (ZTIM1,ZTIM2,ZTIM3)
C
      RDAYL(1)=1.0

      DO 221 JL=1,NLON
      AMU0(JL)=CRAE/(SQRT(AMU0(JL)**2+ZCRAE)-AMU0(JL))
      ZI0(JL) = ZSCT*AMU0(JL)*RDAYL(JL)
C       AMU0(JL)=0.636
C       RDAYL(JL)=0.5
  221 CONTINUE
C
...
C
  360 CONTINUE
c      CALL CLOUD(IZQS,IZCLC,IZQLWP)
      DO 363 JK=IRLEV,1,-1
      DO 362 JL=1,NLON
      ZSW(JL,JK)=ZQS(JL,JK)
      ZCLC(JL,JK)=MIN(MAX(ZCLC(JL,JK),ZEPCLC),1.0-ZEPCLC)
      ZWV(JL,JK)=QM1(JL,JK)
      LO=ZCLC(JL,JK).GT.ZEPCLC
      ZQLWP(JL,JK)=CVMGT(ZQLWP(JL,JK)/ZCLC(JL,JK),0.0,LO)
  362 CONTINUE
  363 CONTINUE
C
...
C
  ...
  513 CONTINUE
      ...
      ZQOFO(JL)=ZOZQ(JL+ISHIFT)*SQRT(ZDPO(JL)**3)/(SQRT(ZDPO(JL)**3)
     *          +ZOZH(JL+ISHIFT))
  514 CONTINUE
      DO 517 JK=1,IRLEV
      DO 515 JL=1,IRLONI
...
C
      ...
      CLWA(JL,JK)=ZQLWP(JL+ISHIFT,JK)*ZCLC(JL+ISHIFT,JK)
  521 CONTINUE
  522 CONTINUE
C
C*         5.3     CALL TO *RADIA*.
...
C
      IF (IBRUEHL .EQ. 1) THEN
         DO 533 JK=1,NLEV
            ...
            CLC(1,JK) = 0.
            CLWA(1,JK) = 0.
            QOF(1,JK) = O3(JK)*DP(1,JK)
 533     CONTINUE
      END IF
C
      DO 538 JK=1,NLEV
        DO 538 JL=1,NLON
          ZTM1(JL,JK)=TM1(JL,JK)
 538  CONTINUE
C
      CALL RADLSW (IRLONI, IRLST, IRLEV, IRLVP1, KRLEV2, KNGL, KNGLP1,
     S             STBO, ZSCT, ZCARDI, ITASK, ZTM1)
C
C
...
C         WRITE(C3EQUIL,192)
C 192     FORMAT(6X,
C     #     ' flxupLW ',6X,' flxdwLW ',6x,' netflxL ',6X,
C     #     ' half pr ',6X,' half te ',6X,' flxdwSW ',6X,
C     #     ' flxupSW ',6X,' netflxS ')
C         WRITE(C3EQUIL,*) '   '
C         WRITE(C3EQUIL,*) '   '
C         WRITE(C3EQUIL,*) '   '
C         DO 180 JL=1,NLON
...
Ccccm            DUMMY = PQOF(JL,JK)/DP(1,JK)*28.96/48.
...
C 180     CONTINUE
C 190     FORMAT(I3,1x,I4,1x,I4,1x,8E14.4)
C         WRITE(C3EQUIL,189)
C 189     FORMAT('Long  Lati  H ',4X,
C     #     'solar h ',6X,'moistur ',6X,' O3conc ',6X,
C     #     'cldfrac ',6x,' netlwh ',6X,' nethrt ',6X,
C     #     'pressur ',6X,'tempera ')
C         WRITE(C3EQUIL,*) '   '
C	  end if

         DO 181 JK=1,NLEVP1
            JKL = NLEVP1 + 1 - JK
            WRITE(27,191) ZFLUX(1,1,JKL), -ZFLUX(1,2,JKL),
     #               -FLT(1,JK),PPMB(1,JKL)*100.
            WRITE(28,191) ZFSDWN(1,JKL),
     #               ZFSUP(1,JKL),
     #               FLS(1,JK),PPMB(1,JKL)*100. 
 191        FORMAT(8E15.6)
 181     CONTINUE
      close(27)
      close(28)
      filename_sw='OUTPUT_ZH_SW'
      filename_lw='OUTPUT_ZH_LW'
      OPEN (77,FILE=filename_sw,FORM='FORMATTED',STATUS='UNKNOWN')
      DO 18888 JL=1,NLON
      	DO 18888 JK=1,NLEV
      	  JKL = NLEV + 1 - JK
          PKASCH = ZHEATR(JL,JKL)
          WRITE(77,19999) PKASCH
18888 CONTINUE
19999 FORMAT(E24.17)

      OPEN (77,FILE=filename_lw,FORM='FORMATTED',STATUS='UNKNOWN')
      	DO 18887 JL=1,NLON
          DO 18887 JK=1,NLEV
            JKL = NLEV + 1 - JK
            PKASCH = -ZCOOLR(JL,JKL)
            WRITE(77,19999) PKASCH
18887    CONTINUE
 1111 CONTINUE
      CLOSE(C3EQUIL)
...
C     ------------------------------------------------------------------
      END
C
      SUBROUTINE CLOUD (IZQS,IZCLC,IZQLWP)
C
C**** *CLOUD* - CALCULATES CLOUD AMOUNTS FOR RADIATION COMPUTATION.
C     J.M.SLINGO     E.C.M.W.F.       10/01/85
C     MODIFIED BY
C     J.-J. MORCRETTE     E.C.M.W.F.     25/02/88
C
C     TO MAKE CLOUD LIQUID WATER CONTENT INDEPENDENT OF VERTICAL
C     RESOLUTION.
C
C     PURPOSE.
C     ________
C
C         THIS ROUTINE COMPUTES CLOUD AMOUNTS WHICH ARE REQUIRED BY THE
C     RADIATION SCHEME FOR COMPUTATION OF THE FLUXES.FOUR SPECIFIC CLOUD
C     TYPES ARE ALLOWED FOR.
C        (1) CONVECTIVE CLOUD(CC).CLOUD AMOUNTS ARE DERIVED FROM
C            TIME AVERAGED PRECIPITATION RATES FROM *KUO* SCHEME.THESE
C            ARE STORED IN EMISM FOR PASSING TO *RADINT*.AN EMPIRICAL
C            SCALING BASED ON CLOUD DATA FROM GATE IS USED TO CONVERT
C            THEM TO CLOUD AMOUNTS.BASE AND TOP LEVELS ARE ALSO PASSED.
C            THE CONVECTIVE CLOUD COVER IS RESTRICTED TO A MAXIMUM OF
C            80%.THE COVER VARIES IN THE VERTICAL BY ASSUMING 0.25*CC
C            AT ALL LEVELS AND 1.0*CC IN THE BASE LEVEL ONLY. WHEN CC
C            EXISTS IN A LAYER THE RELATIVE HUMIDITY OF THE ENVIRONMENT
C            IS ADJUSTED ASSUMING THAT THE AIR IN THE CLOUDY PART IS
C            SATURATED.THIS ADJUSTED RELATIVE HUMIDITY IS USED IN THE
C            CALCULATION OF LAYER CLOUD AMOUNTS.
C
C        (2) HIGH CLOUD(CH).WHEN STRONG DEEP CONVECTION EXISTS (CC>40%
C            AND CC TOP ABOVE 400MB)THEN HIGH (ANVIL CIRRUS) CLOUD
C            COVER IS A LINEAR FUNCTION OF CC.OTHERWISE CH IS COMPUTED
C            AS A FUNCTION OF RELATIVE HUMIDITY.THE CLOUD IS
C            CONSTRAINED TO BE ONLY ONE LAYER THICK.
C
C        (3) MIDDLE CLOUD(CM).AMOUNTS OF MIDDLE LEVEL CLOUD ARE
C            COMPUTED FROM A FUNCTION OF RELATIVE HUMIDITY.CLOUD
C            DEPTH IS CONSTRAINED TO BE ONE SIGMA LAYER THICK AT
C            PRESENT.
C
C        (4) LOW CLOUD(CL).THREE PARAMETERS ARE USED FOR THE
C            CALCULATION OF LOW CLOUD AMMOUNTS.
C            (A) STABILITY.THE VERTICAL GRADIENT OF POTENTIAL
C                TEMPERATURE IS USED TO PREDICT SUBTROPICAL COASTAL
C                STRATUS/SRATOCUMULUS,EXTRATROPICAL ANTICYCLONIC STRATUS
C                AND SUMMERTIME ARCTIC STRATUS.CLOUD COVER IS ALSO
C                DEPENDENT ON THE RELATIVE HUMIDITY AT THE BASE OF THE
C                INVERSION.
C            (B) RELATIVE HUMIDITY AND VERTICAL VELOCITY ARE USED TO
C                PREDICT FRONTAL STRATUS ASSOCIATED WITH RISING AIR.
C
C
C**   INTERFACE.
C     __________
C
C              *CLOUD* IS CALLED FROM *RADINT*. THE ADDRESSES OF QS,
C              CLC AND QLWP ARE ALSO PASSED.
C
C     METHOD.
C     _______
C
C             RELATIVE HUMIDITIES ARE CALCULATED WITHOUT ANY SMOOTHING
C     IN THE VERTICAL.CONVECTIVE CLOUD COVER AT EACH GRID POINT IS
C     EXTRACTED FROM THE *EMISM* FIELD AND APPLIED TO EVERY LEVEL
C     WITHIN THE BASE AND TOP LEVELS OF THE CLOUD.THE BASE AND TOP
C     LEVELS ARE ALSO PASSED IN THE *EMISM* FIELD.THE REMAINING
C     LAYER CLOUDS ARE THEN ADDED TO THE CONVECTIVE CLOUD COVER AT
C     EACH LEVEL WHERE APPROPRIATE.
C             LAYER CLOUD AMOUNTS ARE THEN COMPUTED AS DESCRIBED
C     ABOVE AND ADDED TO THE CONVECTIVE CLOUD WHERE NECESSARY.
C
C     EXTERNALS.
C     __________
C
...
C*     DATA STATEMENTS.
C      ----------------
C    CRITICAL RELATIVE HUMIDITIES FOR LAYER CLOUDS.
C
      CRHH=0.8
      CRHM=0.8
      CRHL=0.8
...
C
C*         1.1     COMPUTE RELATIVE HUMIDITY WITHOUT VERTICAL
C                  SMOOTHING.
C
C
  110 CONTINUE
      DO 112 JK=1,NLEV
      DO 111 JL=1,NLON
      ZRH(JL,JK)=QM1(JL,JK)/ZQS(JL,JK)
      ZRH(JL,JK)=MIN(1.0,MAX(0.0,ZRH(JL,JK)))
  111 CONTINUE
  112 CONTINUE
C
C   COMPUTE ETA AT HALF LEVELS
C
      DO 114 JK=1,NLEV
      DO 113 JL=1,NLON
      ZETA(JL,JK)=APM1(JL,JK)/APHM1(JL,NLEVP1)
  113 CONTINUE
  114 CONTINUE
C
C*       1.2 CLEAR CLOUD ARRAYS.
C
  120 CONTINUE
      DO 122 JK=1,NLEV
      DO 121 JL=1,NLON
...
C
  ...
  125 CONTINUE
C
C*          1.3     SCALE VERTICAL VELOCITY
C
      DO 132 JK=1,NLEV
      DO 133 JL=1,NLON
      VERVEL(JL,JK)=VERVEL(JL,JK)*APM1(JL,JK)
  133 CONTINUE
  132 CONTINUE
C
C*         1.4 CALCULATE POTENTIAL TEPERATURE.
C
  140 CONTINUE
      NLEV2=NLEV/2
      RDCPD=RD/CPD
      DO 142 JK=NLEV,NLEV2,-1
      DO 141 JL=1,NLON
      ZTHETA(JL,JK)=TM1(JL,JK)*(1.0E5/APM1(JL,JK))**RDCPD
  141 CONTINUE
  142 CONTINUE
C
C      _____________________________________________________________
C
C*        2. COMPUTE CONVECTIVE CLOUD COVER FOR EACH LEVEL.
C         _________________________________________________
C
  200 CONTINUE
      IF(NSTEP.EQ.NSTART)GO TO 300
      ZFA=ZSCALE
      DO 201 JL=1,NLON
      IF(ARPRCM(JL).GT.0) THEN
         ZCC(JL)= 0.125*LOG(MAX(1.0E-10,ARPRCM(JL)*ZFA))-1.5
         ZCC(JL)=MIN(CCCC,MAX(0.00,ZCC(JL)))
      ENDIF
      LOC(JL)=(ZCC(JL).GE.0.4).AND.(ZETA(JL,NTOPCM(JL)).LE.0.45)
  201 CONTINUE
C
C*       2.05  ADJUST RELATIVE HUMIDITY FOR PRESENCE OF CONVECTION
C
  205 CONTINUE
      DO 207 JK=1,NLEV
      DO 206 JL=1,NLON
      LO=JK.GE.NTOPCM(JL).AND.JK.LE.NBASECM(JL)
      LO1=JK.EQ.NBASECM(JL)
      ZZRHA=ZRH(JL,JK)-ZCC(JL)*CFCT
      ZZRHB=ZRH(JL,JK)-ZCC(JL)
      ZZRH=CVMGT(ZZRHB,ZZRHA,LO1)
      ZRH(JL,JK)=CVMGT(ZZRH,ZRH(JL,JK),LO)
      ZRH(JL,JK)=MIN(1.0,MAX(0.0,ZRH(JL,JK)))
  206 CONTINUE
  207 CONTINUE
C
C    _____________________________________________________________
C
C*         3.  COMPUTE LAYER CLOUD AMOUNTS.
C          ________________________________
C
  300 CONTINUE
C
C* CALCULATE LAYER CLOUD AMOUNTS
      DO 303 JK=2,NLEVM1
      DO 302 JL=1,NLON
      ZDPHM1=APHM1(JL,JK+1)-APHM1(JL,JK)
      ZQLAY=ZQS(JL,JK)*ZDPHM1
      LOH=ZETA(JL,JK).LE.0.45
      LOM=ZETA(JL,JK).GT.0.45.AND.ZETA(JL,JK).LE.0.80
      LOL=ZETA(JL,JK).GT.0.80
      ZCRH=CRHH
      ZCRH=CVMGT(CRHM,ZCRH,LOM)
      ZCRH=CVMGT(CRHL,ZCRH,LOL)
      ZZCLOUD=MAX(0.,(ZRH(JL,JK)-ZCRH)/(1.-ZCRH))**2
C HIGH CLOUD
      ZCHH=2.0*(ZCC(JL)-0.3)
      LO1=LOC(JL).AND.LOH
      ZCH(JL)=CVMGT(ZCHH,ZCH(JL),LO1)
      ITOPH(JL)=CVMGI(NTOPCM(JL),ITOPH(JL),LO1)
      LO=LOH.AND.ZZCLOUD.GT.ZCH(JL)
      ZCH(JL)=CVMGT(ZZCLOUD,ZCH(JL),LO)
      ITOPH(JL)=CVMGI(JK,ITOPH(JL),LO)
      LO2=LO1.AND.ZCHH.GE.ZZCLOUD
      ZQCON=ZCHH*ZQLAY
      ZQRHC=ZZCLOUD*ZQLAY
      ZQHI=CVMGT(ZQCON,ZQRHC,LO2)
      ZQHIC(JL)=ZQHIC(JL)+CVMGT(ZQHI,0.,LOH)
C MEDIUM CLOUD
      LO=LOM.AND.ZZCLOUD.GT.ZCM(JL)
      ZCM(JL)=CVMGT(ZZCLOUD,ZCM(JL),LO)
      ITOPM(JL)=CVMGI(JK,ITOPM(JL),LO)
      ZQMEC(JL)=ZQMEC(JL)+CVMGT(ZQRHC,0.,LOM)
C LOW CLOUD - NO INVERSION CASES
      LO=VERVEL(JL,JK).GE.0.0
      ZZCLOUD=CVMGT(0.0,ZZCLOUD,LO)
      LO=VERVEL(JL,JK).LT.0.0.AND.VERVEL(JL,JK).GE.-0.1
      ZZCLOUD=CVMGT(ZZCLOUD*VERVEL(JL,JK)*(-10.0),ZZCLOUD,LO)
      LO=LOL.AND.ZZCLOUD.GT.ZCL(JL)
      ZCL(JL)=CVMGT(ZZCLOUD,ZCL(JL),LO)
      ITOPL(JL)=CVMGI(JK,ITOPL(JL),LO)
      ZQRHC=ZZCLOUD*ZQLAY
      ZQLOC(JL)=ZQLOC(JL)+CVMGT(ZQRHC,0.,LOL)
  302 CONTINUE
  303 CONTINUE
C
C*   3.5 COMPUTE LOW CLOUD ASSOCIATED WITH INVERSIONS.
C
  350 CONTINUE
      NLEV2P1=NLEV2+1
      DO 352 JK=NLEVM1,NLEV2P1,-1
      DO 351 JL=1,NLON
      ZDTHDP=(ZTHETA(JL,JK)-ZTHETA(JL,JK-1))*1.0E+02/(APM1(JL,JK)
     *       -APM1(JL,JK-1))
      LO=ZETA(JL,JK).GT.0.80.AND.ZDTHDP.LT.ZDTHMIN(JL)
      ZDTHMIN(JL)=CVMGT(ZDTHDP,ZDTHMIN(JL),LO)
      INVB(JL)=CVMGI(JK,INVB(JL),LO)
  351 CONTINUE
  352 CONTINUE
C
      DO 353 JL=1,NLON
      ZCLL=-6.67*ZDTHMIN(JL)-0.667
      ZCLL=MIN(1.00,MAX(0.00,ZCLL))
      LO=ZCLL.GT.0.0
      JB=INVB(JL)
      ZQLAY=ZQS(JL,JB)*(APHM1(JL,JB+1)-APHM1(JL,JB))
      ZRHB=ZRH(JL,JB)
      LO1=LO.AND.ZRHB.LT.0.8
      ZCLL=CVMGT(ZCLL*(1.-(0.8-ZRHB)*5.),ZCLL,LO1)
      ZCLL=MIN(1.00,MAX(0.00,ZCLL))
      LO=ZCLL.GT.ZCL(JL)
      ZCL(JL)=CVMGT(ZCLL,ZCL(JL),LO)
      ITOPL(JL)=CVMGI(INVB(JL),ITOPL(JL),LO)
      ZQLOC(JL)=ZQLOC(JL)+ZCLL*ZQLAY
  353 CONTINUE
C
      DO 360 JL=1,NLON
      ZCC(JL)=MIN(1.00,MAX(0.00,ZCC(JL)))
      ZCH(JL)=MIN(1.00,MAX(0.00,ZCH(JL)))
      ZCM(JL)=MIN(1.00,MAX(0.00,ZCM(JL)))
      ZCL(JL)=MIN(1.00,MAX(0.00,ZCL(JL)))
  360 CONTINUE
C
C ----------------------------------------------------------------------
C          4.   CLOUD CALCULATIONS NOW COMPLETE.
C               ASCRIBE CLOUD AMOUNT FOR EACH LAYER
C               AND COMPUTE TOTAL CLOUDINESS.
C ----------------------------------------------------------------------
C
  400 CONTINUE
      DO 402 JK=1,NLEVM1
      DO 401 JL=1,NLON
      LO=(JK.GE.NTOPCM(JL)).AND.(JK.LT.NBASECM(JL))
      ZCLC(JL,JK)=CVMGT(ZCLC(JL,JK)+ZCC(JL)*0.25,ZCLC(JL,JK),LO)
      ZQLWP(JL,JK)=ZQLWP(JL,JK)+CSUPSAT*ZCLC(JL,JK)*ZQS(JL,JK)
  401 CONTINUE
  402 CONTINUE
C
      DO 403 JL=1,NLON
      ...
      ITM=ITOPM(JL)
      ITL=ITOPL(JL)
      ZCLC(JL,IBC)=ZCLC(JL,IBC)+ZCC(JL)
      ZQLWP(JL,IBC)=ZQLWP(JL,IBC)+CSUPSAT*ZCLC(JL,IBC)*ZQS(JL,IBC)
      ZCLC(JL,ITH)=ZCLC(JL,ITH)+ZCH(JL)
      ZQLWP(JL,ITH)=ZQLWP(JL,ITH)+CSUPSAT*ZQHIC(JL)
     *             /(APHM1(JL,ITH+1)-APHM1(JL,ITH))
      ZCLC(JL,ITM)=ZCLC(JL,ITM)+ZCM(JL)
      ZQLWP(JL,ITM)=ZQLWP(JL,ITM)+CSUPSAT*ZQMEC(JL)
     *             /(APHM1(JL,ITM+1)-APHM1(JL,ITM))
      ZCLC(JL,ITL)=ZCLC(JL,ITL)+ZCL(JL)
      ZQLWP(JL,ITL)=ZQLWP(JL,ITL)+CSUPSAT*ZQLOC(JL)
     *             /(APHM1(JL,ITL+1)-APHM1(JL,ITL))
      ZCLC(JL,1)=MIN(1.00,MAX(0.00,ZCLC(JL,1)))
      ZCT(JL)=1.0-ZCLC(JL,1)
  403 CONTINUE
C
      DO 412 JK=2,NLEV
      DO 411 JL=1,NLON
      ZCLC(JL,JK)=MIN(1.00,MAX(0.00,ZCLC(JL,JK)))
      ZCT(JL)=ZCT(JL)*(1.0-MAX(ZCLC(JL,JK),ZCLC(JL,JK-1)))/(1.0
     *        -MIN(ZCLC(JL,JK-1),1.-ZEPSEC))
      ZCTLWP(JL)=ZCTLWP(JL)+ZQLWP(JL,JK)
...
C
      DOUBLE PRECISION ZPHM6(NLON),ZPSM6(NLON),ZPHN6(NLON),
     *     ZPSN6(NLON)              
      DOUBLE PRECISION ZEPSH7(NLON),ZEPSH8(NLON),ZEPSH9(NLON)
      DOUBLE PRECISION ZFACTC1(NLON),ZFACTC2(NLON),
     *     ZFACTC3(NLON), ZFACTC4(NLON),ZFACTC5(NLON),
     *     ZFN10(NLON)
      INTEGER IXT(NLON),IXTB(NLON),IXTS(NLON)
C      INTEGER IXTB(NLON),IXTS(NLON)
      DOUBLE PRECISION PGA(NLON,12,2,NLEV),PGB(NLON,12,2,NLEV),
...
C            PQLW  (JL,JK) = PQLWP (JL,JK)
 ...
 97   CONTINUE
C SET LW carbon dioxide to zero here         
C      PCCO2 = 1.0e-7
      ZCCO2 = PCCO2
C
...
C          ZXWV(JL) = AMAX1(PWV(JL,JKL) , ZEPSCQ )
c          ZXOZ(JL) = AMAX1(PQOF(JL,JKL) / PDP(JL,JKL) , ZEPSCO )
 143    CONTINUE
        JKJ=(JK-1)*NG1P1+1
        JKJPN=JKJ+NG1
...
C          ZUP=AMIN1(AMAX1( 0.5*X10E*LOG( ZZABSLY ) +5., 0.), 6.0)
...
C          ZUP   = AMAX1( 0.00 , 5.00 + 0.50 * ZALUP )
...
C
 ...
 2111   CONTINUE
        ...
        DO 213 IG = 1 , NG1
          ...
          KDD = KND + IG
          DO 2112 JL = 1 , NUAER
            ZUU(1,JL) = ZABSCU(1,JL,KND) - ZABSCU(1,JL,KDD)
 2112     CONTINUE
C          
...
C
 2120     CONTINUE
          ...
          DO 2121 JL = 1 , NUAER
            ZUU(1,JL) = ZABSCU(1,JL,KDD) - ZABSCU(1,JL,KNU)
 2121     CONTINUE
...
C
      DO 218 JK = 1 , KFLEV
        ...
        JK1 = JK2 - 1
        DO 216 JL = 1 , NINT
          ZDBDT(1,JL,JK) = ZDBSL(1,JL,JK1) + ZDBSL(1,JL,JK2)
 216    CONTINUE
 218  CONTINUE
C
...
C
 2220 CONTINUE
      ...
      DO 225 JK = 1 , KFLEV-1
        ...
        DO 2222 JL = 1 , NUAER
          ZUU(1,JL) = ZABSCU(1,JL,KN) - ZABSCU(1,JL,KD1)
 2222   CONTINUE
...
C
 2230   CONTINUE
        ...
        DO 224 JKJ=JKP1,KFLEV
          ...
          DO 2232 JL = 1 , NUAER
            ZUU(1,JL) = ZABSCU(1,JL,KN) - ZABSCU(1,JL,KD2)
 2232     CONTINUE
     ...
     *               PPTY(1,1,JKJ), PETY(1,1,JKJ))
          DO 2234 JL = 1 , NTRAER
            ZTT2(1,JL)=(ZTT1(1,JL)+ZTT(1,JL))*0.5
            ZTT1(1,JL)=ZTT(1,JL)
 2234     CONTINUE
...
C
 2250 CONTINUE
      ...
      DO 228 JK=3,KFLEV+1
        ...
        DO 2252 JL = 1 , NUAER
          ZUU(1,JL) = ZABSCU(1,JL,KU1) - ZABSCU(1,JL,KN)
 2252   CONTINUE
...
C
 2260   CONTINUE
        ...
        DO 227 JLK=1,JK-2
          ...
          DO 2262 JL = 1 , NUAER
            ZUU(1,JL) = ZABSCU(1,JL,KU2) - ZABSCU(1,JL,KN)
 2262     CONTINUE
...
C
 ...
 251  CONTINUE
     ...
     *          +ZBSUR(JL,6)
        ...
        ZFU(JL,1)=ZCNSOL+ZBINT(JL,1)-ZDISTU(JL,1)-ZADJU(JL,1)
        ZFUP(JL,1)=ZFU(JL,1)
        PFLUC(JL,1,1)=ZFU(JL,1)
 252  CONTINUE
      ...
      DO 255 JK = 2 , KFLEV+1
        ...
        DO 2521 JL = 1 , NUAER
          ZUU(1,JL) = ZABSCU(1,JL,1) - ZABSCU(1,JL,KN)
2521    CONTINUE
...
C
      ...
      INCLUDE 'zradpar60.f'
      DOUBLE PRECISION PGGA(KDLON,12,2),PGGB(KDLON,12,2),
...
C
 100  CONTINUE
      ...
      DO 101 JL = 1 , 6*KDLON
        ZZ(1) = SQRT (PUU(1,JL))
        ZXN(1) = PGGA(1,JL,1) + ZZ(1)*(PGGA(1,JL,2))
        ZXD(1) = PGGB(1,JL,1) + ZZ(1)*(PGGB(1,JL,2)+ZZ(1))
        PTT(1,JL) = ZXN(1) / ZXD(1)
 101  CONTINUE
      DO 1003 JL = 1 , KDLON
C
        ZZ(JL) = SQRT (PUU(JL,8))
...
C-  OZONE ABSORPTION                                                      
C                                                                         
c        ZX = AMAX1(1.0E-15, PUU(JL,12))
c        ZY = AMAX1(1.0E-15, PUU(JL,13))
...
C                                                                         
      ...
      ZXCH4 = MAX(1.0E-12, PUU(JL,19))
      ZYCH4 = MAX(1.0E-12, PUU(JL,20))
...
C                                                                         
      ZXN2O = MAX(1.0E-12, PUU(JL,21))
      ZYN2O = MAX(1.0E-12, PUU(JL,22))
...
C
      INCLUDE 'zradpar60.f'
      DOUBLE PRECISION ZTM1(NLON,NLEV)
C
C-----------------------------------------------------------------------
C
C     -----------------------------------------------------------------
...
C
      DO 202 JK = 2 , KFLEV-1
         ...
         ZLWGKG = CVMGT(3.*PQLWP(JL,JKL)*1000./PCLFR(JL,JKL),0.0,LO1)
...
C
C      IF(IMP.LT.4) THEN
C         WRITE(NOUT,889) (RT1(IG1),WG1(IG1),IG1=1,NG1)
CC     DO 210 JL = 1 , KDLON
C      JL = 1
C      WRITE(NOUT,891)(ZPMB(JL,JK),  JK = 1 , KFLEV+1)
C      WRITE(NOUT,891)(ZTL (JL,JK),  JK = 1 , KFLEV+1)
C      WRITE(NOUT,883)(PWV(JL,JK),  JK=KFLEV,1,-1)
C      WRITE(NOUT,884)(PQOF (JL,JK),JK=KFLEV,1,-1)
C      WRITE(NOUT,884)(ZOZ (JL,JK),JK = 1 , KFLEV)
C      WRITE(NOUT,885)(ZTAVE(JL,JK), JK = 1 , KFLEV)
C      WRITE(NOUT,889)(ZCLDSW(JL,JK),  JK = 1 , KFLEV)
C      WRITE(NOUT,889)(ZCLDLW(JL,JK), JK = 1 , KFLEV)
C      WRITE(NOUT,889)(PCLFR(JL,JK), JK=KFLEV,1,-1)
C      WRITE(NOUT,889)(PQLWP(JL,JK),  JK=KFLEV,1,-1)
C      WRITE(NOUT,889)(ZTAU(JL,1,JK),  JK = 1 , KFLEV)
C      WRITE(NOUT,889)(ZOMEGA(JL,1,JK),JK = 1 , KFLEV)
C 210  CONTINUE
C      WRITE(NOUT,887) PCARDI,PSIG,PSCT,G,CPD
C
C      END IF
C
C     ------------------------------------------------------------------
...
C
C
C 883  FORMAT(2X,16E9.2)
C 884  FORMAT(4X,18F7.4)
C 885  FORMAT(4X,18F7.1)
C 887  FORMAT(1X,10E14.6)
C 889  FORMAT(4X,18F7.3)
C 891  FORMAT(1X,18F7.1)
C
C     --------------------------------------------------------------
C
...
C         NTRAER = 14
 ...
 306     CONTINUE
     ...
     S          PSIG, PSCT, PCARDI, ITASK)
      ...
      ZDFNET = ZFLUX(JL,1,JK+1) + ZFLUX(JL,2,JK+1)
     S              -ZFLUX(JL,1,JK  ) - ZFLUX(JL,2,JK  )
      ZCOOLR1(JL,JK) = CDAY * ZDFNET / PDP(JL,JKL)
 6112 CONTINUE
 6122 CONTINUE
      RETURN
      END
...
C
      INCLUDE 'zradpar60.f'
      DOUBLE PRECISION XL2, XL3, ALOS
      DOUBLE PRECISION FOXDWN(IRLST, IRLVP1)
C     ------------------------------------------------------------------
C
C DIR$ VFUNCTION ALOG, EXP
C     -----------------------
...
C
         DO 224 JL = 1 , KDLON
            ZCGAZ(JL,JK) = CVMGT( 0.0, ZCGAZ(JL,JK) / ZPIZAZ(JL,JK),
     S                            IAER.EQ.0)
             ZPIZAZ(JL,JK) = CVMGT( 1.0, ZPIZAZ(JL,JK) / ZTAUAZ(JL,JK),
     S                            IAER.EQ.0)
...
C
      DATA (GA(11,11,JC),JC=1,3) /
     S 0.64195381E-02, 0.38088067E+00, 0.00000000E+00/
      DATA (GB(11,11,JC),JC=1,3) /
     S 0.64195381E-02, 0.38906156E+00, 0.10000000E+01/
      DATA (GA(11,12,JC),JC=1,3) /
     S-0.51581287E+00, 0.47751980E+00, 0.00000000E+00/
      DATA (GB(11,12,JC),JC=1,3) /
     S-0.51581287E+00,-0.60783385E+00, 0.10000000E+01/
C
...
c
      DO 1000 I=1,40
        DO 1000 J=1,31
          DO 1000 K=1,2
            ALP(I,J,K)=ALP(I,J,K)*1.E-04
            BET(I,J,K)=BET(I,J,K)*1.E-07
 1000 CONTINUE
      DO 1001 I=1,38
        DO 1001 J=1,31
          AO3(I,J)=AO3(I,J)*1.E-04
          BO3(I,J)=BO3(I,J)*1.E-06
          O3A(I,J)=O3A(I,J)*1.E-04
          O3B(I,J)=O3B(I,J)*1.E-06
 1001 CONTINUE
      RETURN
      END
C
...
C
      RETURN
      END
      SUBROUTINE SOLANG (PTIM1,PTIM2,PTIM3)                               SOLANG
C                                                                         SOLANG
C**** *SOLANG* - FOR SOLAR ZENITH ANGLE AND RELATIVE DAYLENGTH.           SOLANG
C                                                                         SOLANG
C     J.F.GELEYN     E.C.M.W.F.     03/06/82.                             SOLANG
C                                                                         SOLANG
C     PURPOSE.                                                            SOLANG
C     --------                                                            SOLANG
C                                                                         SOLANG
C          THIS ROUTINE GIVES DIFFERENT RESULTS DEPENDING ON A LOGICAL    SOLANG
C     SWITCH. IF LDIUR IS TRUE ONE OBTAINS ACTUAL SOLAR ZENITH ANGLES     SOLANG
C     AND VALUES OF ONE OR ZERO DEPENDING ON THE SIGN OF THE FORMER. IF   SOLANG
C     LDIUR IS FALSE ONE GETS THE SAME ANSWERS AT ALL POINTS, I.E. MEAN   SOLANG
C     VALUE OF THE DAYTIME SOLAR ZENITH ANGLE AND RELATIVE LENGTH OF      SOLANG
C     THE DAY.                                                            SOLANG
C                                                                         SOLANG
C**   INTERFACE.                                                          SOLANG
C     ----------                                                          SOLANG
C                                                                         SOLANG
C          *SOLANG* IS CALLED FROM *RADMOD* AND FROM *RADHEAT*.           SOLANG
C          THERE ARE THREE DUMMY ARGUMENTS: *PTIM1*, *PTIM2* AND *PTIM3*  SOLANG
C     ARE LATITUDE DEPENDENT PARAMETER ABOUT THE SUN'S POSITION.          SOLANG
C          THE ROUTINE RETURNS SOLAR ZENITH ANGLES AND RELATIVE DAY       SOLANG
C     LENGTHS TO THE LONG TERM STORAGE.                                   SOLANG
C                                                                         SOLANG
C     METHOD.                                                             SOLANG
C     -------                                                             SOLANG
C                                                                         SOLANG
C          STAIGHTFORWARD IN THE CASE "ON". FOR THE CASE "OFF" THE        SOLANG
C     TYPE OF "ON" COMPUTATION IS REPEATED  WITH 128 POINTS AND THE       SOLANG
C     RELEVANT MEAN VALUES ARE COMPUTED AND STORED.                       SOLANG
C                                                                         SOLANG
C     EXTERNALS.                                                          SOLANG
C     ----------                                                          SOLANG
C                                                                         SOLANG
...
C     ----------                                                          SOLANG
C                                                                         SOLANG
C          SEE RELEVANT PART OF THE DOCUMENTATION.                        SOLANG
C                                                                         SOLANG
      INCLUDE 'zradpar60.f'
C                                                                         SOLANG
C     ----------                                                          SOLANG
C                                                                         SOLANG
      DOUBLE PRECISION ZMU0(128),ZRDAYL(128)
C                                                                         SOLANG
C     ------------------------------------------------------------------  SOLANG
C                                                                         SOLANG
C*         1.     LOCATE AND POSITION SPACE.                              SOLANG
C                 ------ --- -------- ------                              SOLANG
C                                                                         SOLANG
  100 CONTINUE                                                            SOLANG
C                                                                         SOLANG
...
C     ------------------------------------------------------------------  SOLANG
C                                                                         SOLANG
C*         2.     COMPUTATIONS IF DIURNAL CYCLE "ON".                     SOLANG
C                 ------------ -- ------- ----- -----                     SOLANG
C                                                                         SOLANG
  200 CONTINUE                                                            SOLANG
C                                                                         SOLANG
C***                                                                      SOLANG
      IF(LDIUR) THEN                                                      SOLANG
C***                                                                      SOLANG
      DO 201 JL=1,NLON                                                    SOLANG
      AMU0(JL)=ZTIM1+ZTIM2*COSLON(JL)+ZTIM3*SINLON(JL)                    SOLANG
      LO=AMU0(JL).GE.0.                                                   SOLANG
      AMU0(JL)=CVMGT(AMU0(JL),0.0,LO)
      RDAYL(JL)=CVMGT(1.0,0.0,LO)
  201 CONTINUE                                                            SOLANG
C                                                                         SOLANG
C     ------------------------------------------------------------------  SOLANG
C                                                                         SOLANG
C*         3.     COMPUTATIONS IF DIURNAL CYCLE "OFF".                    SOLANG
C                 ------------ -- ------- ----- ------                    SOLANG
C                                                                         SOLANG
  300 CONTINUE                                                            SOLANG
C                                                                         SOLANG
...
C***                                                                      SOLANG
      ...
      LO=ZMU0(JL).GE.0.                                                   SOLANG
      ZMU0(JL)=CVMGT(ZMU0(JL),0.0,LO)                                     VECTIF
      ZRDAYL(JL)=CVMGT(1.0,0.0,LO)                                        VECTIF
  301 CONTINUE                                                            SOLANG
      ZS1=SIGMA(128,ZMU0(1),1)                                            SOLANG
      ZS2=SIGMA(128,ZRDAYL(1),1)                                          SOLANG
      IF(ZS2.NE.0.) THEN                                                  SOLANG
      ZS1=ZS1/ZS2                                                         SOLANG
      ZS2=ZS2/128.                                                        SOLANG
      END IF                                                              SOLANG
      DO 302 JL=1,NLON                                                    SOLANG
      AMU0(JL)=ZS1                                                        SOLANG
      RDAYL(JL)=ZS2                                                       SOLANG
  302 CONTINUE                                                            SOLANG
C***                                                                      SOLANG
...
C Input day number
      ...
      DATA NDAY/0,31,59,90,120,151,181,212,243,273,304,334,365/
...
C   ------------------
      ...
      DO 100 MM=1,12
          IF (JDAY.GT.NDAY(MM).AND.JDAY.LE.NDAY(MM+1)) THEN
             MDAY = JDAY-NDAY(MM)
             MONTH = MNTH(MM)
             GOTO 200
          ENDIF
  ...
  200 CONTINUE
      ...
      WRITE(6,111) MONTH, MDAY
 111  FORMAT(4X,' month: ',A4,10X,'day: ',I3)
C
      END
C***************************************************************
C
...
C***************************************************************
C
       DOUBLE PRECISION FUNCTION CVMGT(Z1, Z2, LO)
C
       IMPLICIT NONE
C
...
C***************************************************************
C
       DOUBLE PRECISION FUNCTION CVMGM(Z1, Z2, Z3)
C
       IMPLICIT NONE
C
...
C***************************************************************
C
       DOUBLE PRECISION FUNCTION SSUM(III, Z1V, IIINCX)
C
       IMPLICIT NONE
C
       INTEGER I, III, IIINCX
       DOUBLE PRECISION Z1V(III*IIINCX)
C
       SSUM = 0.
       DO 1 I=1,III*IIINCX,IIINCX
          SSUM = SSUM + Z1V(I)
 1     CONTINUE
C
       END
...
C***************************************************************
C
       DOUBLE PRECISION FUNCTION SIGMA(N, SX, INCX)
C
       IMPLICIT NONE
C
       INTEGER N, INCX, I
       DOUBLE PRECISION SX(N*INCX)
C
       SIGMA = 0.
       DO 1 I=1,N
          SIGMA = SIGMA + SX(I*INCX)
 1     CONTINUE
C
       END
