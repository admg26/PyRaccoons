...
C
      PARAMETER (NRLEV=100,NUA=34,NINT=6,NTRA=34, NLAT=1,
C      PARAMETER (NRLEV=21,NUA=34,NINT=6,NTRA=34, NLAT=1,
cccm      PARAMETER (NRLEV=37,NUA=18,NINT=6,NTRA=19, NLAT=1c
cccm     #           NRLEVP1=NRLEV+1,NRLST=1,ITIM=10,NLEV=NRLEV,
     #           NRLEVP1=NRLEV+1,NRLST=1,NLEV=NRLEV,
     #           NLP2=NRLST, NLEVP1=NRLEVP1, NLON=NRLST,
     #           NRPARTI=1)
CITIM=1,
C
      DOUBLE PRECISION YCLOCK, XLAT(NLAT), Z0, DZ, ALBMPAR, SNPAR,
...
C
      PARAMETER (YCLOCK=3.14159, Z0=0., CTFREEZ=271.26,
...
C
      PARAMETER (LDIUR=.TRUE., LOLPAR=.TRUE.)
C       PARAMETER (LDIUR=.FALSE., LOLPAR=.TRUE.)
C
      INTEGER  C2TEMP
      CHARACTER*64  MIDTMP
...
C
      DOUBLE PRECISION       CVMGT, SIGMA2
      INTEGER    CVMGI
C
C    definitons of the subroutine cloud
C
      DOUBLE PRECISION RDCPD, CCCC, CFCT, CRHH, CRHM, CRHL, CCLWMR
C
C    missing definitions in morcrette
C
      DOUBLE PRECISION   PSIG, PSCT, PCCO2, PCARDI, PTIM1, PTIM2, PTIM3
      DOUBLE PRECISION ZCCO2
...
C
      ...
      INTEGER Niter,iconv,count1,count2,NIter_max,pp
      LOGICAL  Convgt, ConvgF
  
      DOUBLE PRECISION:: 
     & maxdtold,maxdt,dt,fac,flux_tropop,F_old

C      DOUBLE PRECISION(kind=8) Told(NRLEV),temp_diff(NRLEV),
      DOUBLE PRECISION T_old(NRLEV),temp_diff(NRLEV),
     &  t_initial(NRLEV),ZCOOLIN(NRLEV),ZCOOLSW(NRLEV),
     &  t_lev_initial(NRLEV+1),DelQ(NRLEV,40000),DelT(NRLEV,40000)
     &     ,DelQ1(NRLEV,40000),DelQ2(NRLEV,40000),DelQ3(NRLEV,40000)
     &     ,temp(NRLEV+1,40000),FUP_time(NRLEV+1,40000)
     &     ,FDN_time(NRLEV+1,40000),cloud_time(NRLEV,40000)
...
C
     ...
     R CDISSEM
C
C      DOUBLE PRECISION       COSRAD(128),        SINRAD(128)
C
C
C       ----------------------------------------------------------------
C
...
C     -----------------------------------------------------------------
C
      DOUBLE PRECISION TAUA(2,5), PIZA(2,5), CGA(2,5), CAER(5,5), CAEROS
C
C     -----------------------------------------------------------------
C
C     COMMON *YOMLW*
...
C
      DOUBLE PRECISION  RT1(2),WG1(2),AT(12,3),BT(12,3),CNTNU(4),O1H
     S ,OCT(4),TREF,XP(6,6),TSTAND,GA(11,24,3),GB(11,24,3)
     S ,PTY(11,20),ETY(11,20),TINTP(11),TSTP,O2H,PIALF0
C
      DOUBLE PRECISION  TB(40,31,2),ALP(40,31,2),BET(40,31,2),
...
C
      DOUBLE PRECISION          CDAY,CCO2,CH2O,DIFF,X10E
     S             ,ZEELOG,ZEPSC,ZEPSCO,ZEPSCQ,ZEPSCT,ZEPSCW
C     -----------------------------------------------------------------
C*COMDECK YOMSW
C     -----------------------------------------------------------------
...
C
      DOUBLE PRECISION  APAD(2,3,7),BPAD(2,3,7),D(2,3),CRAY(2,6),SUN(2)
C     -----------------------------------------------------------------
 
C
C  --------------------------------------------------------------------
C
      DOUBLE PRECISION   TTT(1,NLAT,NRLEV,12)
C
C        0.1  "RADINT POINTERS"
C             -----------------
C
      DOUBLE PRECISION   TM1(NLP2,NLEV),
     *       TSM1M(NLP2),
     *       QM1(NLP2,NLEV),
     *       SNM1M(NLP2),
     *       ALBM(NLP2),
     *       AVVELM(NLP2)
      DOUBLE PRECISION   SLMM(NLP2),
     *       APHM1(NLP2,NLEVP1),
     *       APM1(NLP2,NLEV)
C
      LOGICAL LOLAND(NLP2)
...
C
      DOUBLE PRECISION   ZTI(NLP2,NRLEVP1),
     *       ZCTI(NLP2,NRLEVP1),
     *       ZCPH(NLP2,NRLEVP1),
     *       ZWV(NLP2,NRLEV),
     *       ZSW(NLP2,NRLEV),
     ...
     *       ZCALSO(NLP2)
C
      DOUBLE PRECISION   ZOZQ(NLP2),
     #       ZOZH(NLP2)
C
      DOUBLE PRECISION   ZTRSOL(NLP2,NLEVP1),
     *       ZEMTER(NLP2,NLEVP1)
C
C     FINAL RESULTS ARE STORED ON:
C
      DOUBLE PRECISION   TRSOLM(NLP2,NLEVP1),
     *       EMTERM(NLP2,NLEVP1)
C
      DOUBLE PRECISION   AMU0(NLP2),
     *       RDAYL(NLP2)
C
C     VARIABLES WITHIN THE SPLITTED LATITUDE LOOP.
C
      DOUBLE PRECISION   CLC(NRLST,NRLEV),
     *       TI(NRLST,NRLEVP1),
     *       DP(NRLST,NRLEV),
     *       AADS(NRLST,NRLEV),
     *       AEQ1(NRLST,NRLEV),
     ...
     *       ZAEQSN(NRLST),
     *       ZQOFN(NRLST)
      DOUBLE PRECISION   FLT(NRLST,NRLEVP1),
     *       FLS(NRLST,NRLEVP1)
C
C     DIAGNOSTIC VARIABLES.
C
      DOUBLE PRECISION   DIAG(NLEVP1,4),
     *       DIA1(7),
     *       ZDIAG(NLEVP1,10),
     *       ZDIA1(21),
     *       ZDIA(NLON),
...
C
      DOUBLE PRECISION     PALBSN  (IRLST)
C
C*       0.2   "LOCAL" ARRAYS
C              --------------
      DOUBLE PRECISION ZABSCU(IRLST ,NUA, 3*NRLEV+1),
...
C     S     IXT   (IRLST , IRLVP1,IRLVP1),
     S     IXTSU (IRLST , IRLVP1)       ,
     S     IXTTP (IRLST , IRLVP1)       ,
     S     IMX   (IRLST)                ,
     S     IMXP  (IRLST)
C
      DOUBLE PRECISION ZOZ   (IRLST , IRLEV)        ,
     S     PPMB  (IRLST , IRLVP1)       ,
     S     ZRES  (IRLST)                ,
     S     PB    (IRLST, 6 , IRLVP1)    ,
...
!      DOUBLE PRECISION(kind=8) PTAVE (IRLST , IRLEV),        
!     S     PTL   (IRLST , IRLVP1)       
...
C
      INTEGER KT    (IRLST)
      DOUBLE PRECISION    PUU   (IRLST , NUA  )     ,
     S        PTT   (IRLST , NTRA )     ,
     S        ZZ    (IRLST)
C
C     -------------------------------------------------------- ---------
C
      DOUBLE PRECISION  PCLFR  (IRLST , IRLEV)       ,
!     S      PT     (IRLST , IRLVP1)       ,
     S      PQOF   (IRLST , IRLEV)          ,
     S      PQLWP  (IRLST , IRLEV)          ,
     S      PEMIS  (IRLST)

c      REAL(kind=8)      PT(IRLST , IRLVP1)       
      DOUBLE PRECISION      PT(IRLST , IRLVP1)       
C
      DOUBLE PRECISION
     S      PFLT   (IRLST , IRLVP1)
C
      DOUBLE PRECISION  ZALBSU(IRLST , 2 )           ,
     S      ZCG   (IRLST , 2 , IRLEV)    ,
     S      ZCLDLW(IRLST , IRLEV)        ,
     S      ZCLDSW(IRLST , IRLEV)        ,
     S      ZCOOLR(IRLST , IRLEV)        ,
     S      ZCOOLR1(IRLST , IRLEV)        ,
     S      ZDT0  (IRLST)                ,
...
C      REAL(kind=8)   ZTL(IRLST , IRLVP1),ZTAVE (IRLST , IRLEV)       
      DOUBLE PRECISION   ZTL(IRLST , IRLVP1),
     S      ZTAVE (IRLST , IRLEV)       

C     -----------------------------------------------------------------
C
C
      DOUBLE PRECISION   PDP   (IRLST , IRLEV)     ,
     S       PWV   (IRLST , IRLEV)     ,
     S       PFLS  (IRLST , IRLVP1)    ,
     S       PPSOL (IRLST        )     ,
     S       PRMU0 (IRLST        )     ,
     ...
     S       POZ   (IRLST , IRLEV)
C
      DOUBLE PRECISION   PFDOWN(IRLST , IRLVP1)    ,
     S       PFUP  (IRLST , IRLVP1)
C
      DOUBLE PRECISION   ZAKI  (IRLST , 2 )        ,
     S       ZCGAZ (IRLST , IRLEV)     ,
     S       ZCNEB (IRLST , IRLEV)     ,
     S       ZC1I  (IRLST , IRLVP1)    ,
     S       ZDSC  (IRLST , IRLEV)     ,
...
C   --------------
C
      DOUBLE PRECISION   ZRH(NLP2,NLEV),
     *       ZCH(NLP2),
     *       ZCM(NLP2),
     *       ZCL(NLP2),
     *       ZCT(NLP2),
     *       ZCC(NLP2)
      INTEGER NTOPCM(NLP2),
     *        NBASECM(NLP2)
      DOUBLE PRECISION   ARPRCM(NLP2)
      INTEGER ITOPH(NLP2),
     *        ITOPM(NLP2),
     *        ITOPL(NLP2)
      LOGICAL LOC(NLP2)
      DOUBLE PRECISION   ZQS(NLP2,NLEV),
     *       ZCLC(NLP2,NLEV),
     *       ZQLWP(NLP2,NLEV)
C
      DOUBLE PRECISION   VERVEL(NLP2,NLEV),
     *       ZTHETA(NLP2,NLEV),
     *       ZETA(NLP2,NLEV),
     *       ZDTHMIN(NLP2)
      INTEGER INVB(NLP2)
      DOUBLE PRECISION   ZCTLWP(NLP2),
     *       ZQLOC(NLP2),
     *       ZQMEC(NLP2),
     *       ZQHIC(NLP2)
C
